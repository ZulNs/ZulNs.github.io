...
function Calendar(isHijr,year,month,firstDay,lang,theme,tmout){
	...
	menuCloseElm=createElm('button','w3-leftbar w3-border-light-grey w3-bar-item w3-button w3-ripple'),
	...
	createStyle=function(){
		...
		str+='.zulns-calendar .w3-button.w3-border-dark-grey:focus{color:#fff!important;background-color:#616161!important}';
		stl=createElm('style',null,str);
		stl.id='ZulNsCalendarStyle';
		stl.type='text/css';
	...
	createCal=function(){
		...
		menuWrapElm.appendChild(accFirstDayElm);
		...
		addEvt(menuCalModElm,'mouseenter',onHover);
		addEvt(menuFirstDayElm,'mouseenter',onHover);
		addEvt(menuTodayElm,'mouseenter',onHover);
		addEvt(menuNewThemeElm,'mouseenter',onHover);
		addEvt(menuAboutElm,'mouseenter',onHover);
		addEvt(menuCloseElm,'mouseenter',onHover);
		for(var i=0;i<7;i++){
			var el=createElm('button','w3-leftbar w3-border-white w3-bar-item w3-white w3-button w3-ripple');
			el.setAttribute('firstday',i);
			addEvt(el,'click',onSelFirstDay);
			addEvt(el,'mouseenter',onHover);
	...
	createWdayTitle=function(){
		...
		replaceClass(el,'w3-button w3-ripple','w3-disabled w3-transparent');
	...
	createDates=function(){
		...
		for(var i=1;i<=ppdr+pcdr+pndr;i++){
			...
			if(i<=ppdr||ppdr+pcdr<i){grid.className+=' w3-disabled';grid.style.cursor='default'}
				...
				replaceClass(menuTodayElm,'w3-button w3-ripple','w3-disabled w3-transparent');
				menuTodayElm.style.cursor='default';
				menuTodayElm.tabIndex=-1;
	...
	hideMenu=function(){
		if(isAccOpened)onFirstDay();
		replaceClass(menuWrapElm,' w3-show','');
		replaceClass(menuBtnElm,'focus ','')
	},
	onClickMenu=function(){
		if(menuWrapElm.className.indexOf('w3-show')==-1){
			menuBtnElm.className='focus '+menuBtnElm.className;
			menuWrapElm.className+=' w3-show';
		}else hideMenu()
	},
	onHover=function(ev){
		ev=ev||window.event;var el=ev.target||ev.srcElement;if(el.className.indexOf('w3-disabled')==-1)el.focus()
	},
	...
	onSelFirstDay=function(ev){
		...
		if(el.style.cursor!='default'){self.setFirstDayOfWeek(el.getAttribute('firstday'));applyTodayTmout()}
		else el.blur()
	},
	onDispToday=function(){if(!isDispToday)self.today();else menuTodayElm.blur()},
	...
	this.setFirstDayOfWeek=function(fdow){
		...
		if(fdow!=firstDay){
			...
			replaceClass(el,'w3-disabled w3-transparent','w3-button w3-ripple');
			el.style.cursor='';
			el.tabIndex=0;
			firstDay=fdow;
			recreateWdayTitle();
