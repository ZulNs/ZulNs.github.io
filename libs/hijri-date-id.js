 ...
 * @namespace HijriDate
 *
 * revised to Indonesian on 30 April 2017
 * .getTimezoneOffset() method also added
 */
...
function HijriDate(year, month, date, hour, minute, second, millisecond) {
	...
	this.getTimezoneOffset = function() {
		return gregDate.getTimezoneOffset();
	}
	...
	this.getFullYearString = function() {
		return (year > 0) ? HijriDate.toDigit(year, 4) + " H" : HijriDate.toDigit(Math.abs(year - 1), 4) + " SH";
	};
...
HijriDate.monthNames = ["Muharram", "Shafar", "Rabi'ul awwal", "Rabi'ul akhir", "Jumadil awwal", "Jumadil akhir",
					   "Rajab", "Sya'ban", "Ramadhan", "Syawwal", "Dzulqa'dah", "Dzulhijjah"];

HijriDate.monthShortNames = ['Muh', 'Sha', 'Raw', 'Rak', 'Jaw', 'Jak', 'Raj', 'Sya', 'Ram', 'Syw', 'Dzq', 'Dzh'];

//HijriDate.weekdayNames = ["Ahad", "Ithnin", "Thulatha", "Arba'a", "Khams", "Jumu'ah", "Sabt"];
HijriDate.weekdayNames = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', "Jum'at", 'Sabtu' ];

//HijriDate.weekdayShortNames = ['Ahd', 'Ith', 'Thu', 'Arb', 'Kha', 'Jum', 'Sab'];
HijriDate.weekdayShortNames = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'];
...
Date.monthShortNames = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des'];
...
Date.prototype.getDayName = function(day) {
	return HijriDate.weekdayNames[(day === undefined) ? this.getDay() : parseInt(day)];
};

Date.prototype.getDayShortName = function(day) {
	return HijriDate.weekdayShortNames[(day === undefined) ? this.getDay() : parseInt(day)];
};
...
Date.prototype.getFullYearString = function() {
	var y = this.getFullYear();
	return (y > 0) ? HijriDate.toDigit(y, 4) : HijriDate.toDigit(Math.abs(y - 1), 4) + " SM";
};
