...
/*************************************************************
 * Pure CSS3 Dynamic Pyramid Animation With Background Image *
...
var LENGTH = 140;
var EXPAND = 100;
var IMG_FRONT   = '../images/pyramid/front.png';
var IMG_BACK    = '../images/pyramid/back.png';
var IMG_LEFT    = '../images/pyramid/left.png';
var IMG_RIGHT   = '../images/pyramid/right.png';
var IMG_BOTTOM  = '../images/pyramid/bottom.png';
var BACKFACE_FB = '../images/pyramid/bffb.png';
var BACKFACE_LR = '../images/pyramid/bflr.png';
var BACKFACE_B  = '../images/pyramid/bfb.png';
var NAME = 'pyramid';
...
function createFace(w, h, rx, ry, ty, tz, rx2, urx, img, ix, iy, bf, bfx, bfy, cname) {
...
function createModel() {
	...
	var frx = Math.atan2(LENGTH / 2, HEIGHT);
	var srx = Math.atan2(WIDTH / 2, HEIGHT);
	_frontTH = HEIGHT / Math.cos(frx);
	_sideTH = HEIGHT / Math.cos(srx);
	var fdh = (_frontTH - HEIGHT) / 2;
	var sdh = (_sideTH - HEIGHT) / 2;
	frx = frx * 180 / Math.PI;
	srx = srx * 180 / Math.PI;
	model.appendChild(createFace(WIDTH, _frontTH, 0, 179.99, -fdh, LENGTH / 2, frx, -90, IMG_BACK, 0, 0, BACKFACE_FB, 0, 0, 'back'));
	model.appendChild(createFace(LENGTH, _sideTH, 0, -90, -sdh, WIDTH / 2, srx, -90, IMG_LEFT, 0, 0, BACKFACE_LR, 0, 0, 'left'));
	model.appendChild(createFace(LENGTH, _sideTH, 0, 90, -sdh, WIDTH / 2, srx, -90, IMG_RIGHT, 0, 0, BACKFACE_LR, 0, 0, 'right'));
	model.appendChild(createFace(WIDTH, LENGTH, -90, 0, 0, HEIGHT / 2, 0, 0, IMG_BOTTOM, 0, 0, BACKFACE_B, 0, 0, 'bottom'));
	model.appendChild(createFace(WIDTH, _frontTH, 0, 0, -fdh, LENGTH / 2, frx, -90, IMG_FRONT, 0, 0, BACKFACE_FB, 0, 0, 'front'));
...
function showImageInfo() {
	alert(
		...
		IMG_FRONT + ' : ' + WIDTH + ' x ' + _frontTH.toFixed(2) + '\n' +
		IMG_BACK + ' : ' + WIDTH + ' x ' + _frontTH.toFixed(2) + '\n' +
		IMG_LEFT + ' : ' + LENGTH + ' x ' + _sideTH.toFixed(2) + '\n' +
		IMG_RIGHT + ' : ' + LENGTH + ' x ' + _sideTH.toFixed(2) + '\n' +
...
function init() {
	...
	_productToRadians = 2 * Math.PI / Math.max(_frontTH + _frontTH + LENGTH, _sideTH + _sideTH + WIDTH);
}
